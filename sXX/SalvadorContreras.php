
	<?php include'../Layouts/header.php'?>
    
    <title>Salvador Contreras Balderas</title>
    </head>
    		<section>
										<br>
										  <div class="row">
													<div id='timeline-embed' style="width: 100%; height: 500px;" start_at_slide="84"></div>
											</div>
									</section><br><br>
	<body class="subpage">

			<!-- Content -->
				<div id="content-wrapper">
					<div id="content">
						<div class="container">
							<div class="row">
								<div class="3u 12u(mobile)">

									<!-- Left Sidebar -->
										<section>
											<header><img src="../images/personajes/SalvadorContreras.jpg" width="230" height="350" alt=""/></header>
</section>
										<section>
											<header>
												<h2>Referencias</h2>
											</header>
<ul class="link-list last-child">
			  <li>Lozano-Vilano, M. (ed.) 2002. Libro Jubilar en Honor al Dr. Salvador Contreras. Universidad Autónoma de Nuevo León. Monterrey, Nuevo León</li>
			  <li>Contreras-Balderas, S., M.L. Lozano-Vilano y M.E García-Ramírez. 1995. Peces de la cuenca geohidrológica del río Bravo. Atlas. Bioconservación y Universidad Autónoma de Nuevo León. Monterrey.</li>
			  <li>Valdez-Moreno, M.E., y A. Contreras. 2009. In memoriam: Dr. Salvador Contreras Balderas (1936-2009) Hidrobiológica 9(1).</li>
		      </ul>
                                          </section>
								</div>
                                
                                <div class="6u 12u(mobile) important(mobile)">

									<!-- Main Content -->
										<section>
											<header>
												<h2>Salvador Contreras Balderas</h2>
												<h3>1936 - 2009</h3>
											</header>
<p> Nací el 19 de febrero de 1936 en la Ciudad de México. Mi padres fueron Luis Contreras Díaz y Victoria Balderas Freeman. Nos mundamos a Monterrey, Nuevo León antes que yo entrara a la primaria. Fui el primero de siete hermanos. A mi papá y a mi tío Salvador les gustaba salir de excursión al campo y lo hacían con frecuencia en compañía de toda la familia. Me encantaba irme a nadar a los ríos y traerme peces a la casa.</p>
<p>Desde la prepa me interesé en criar peces y algunos de ellos ¡eran endémicos de Nuevo León! Me encaminaba hacia la carrera de Laboratorista Químico Biólogo, pero conocí al Dr. Eduardo Aguirre Pequeño quien me abrió los ojos y entré a la carrera de biólogo en la Universidad Autónoma de Nuevo León. En un curso de posgrado conocí a maestros como <a href="http://www.biodiversidad.gob.mx/pais/curiosos/sXIX/EnriqueRioja.php">Enrique Rioja, </a>Eduardo Caballero, <a href="http://www.biodiversidad.gob.mx/pais/curiosos/sXX/JoseAlvarez.php">José Álvarez del Villar, </a><a href="http://www.biodiversidad.gob.mx/pais/curiosos/sXX/MariaElenaCaso.php">Maria Elena Caso, </a><a href="http://www.biodiversidad.gob.mx/pais/curiosos/sXX/AlejandroVillalobos.php">Alejandro Villalobos, </a>Zacarías Popovici, Jorge Carranza, entre muchos otros. Desde el 3er año de la carrera empecé a trabajar como profesor auxiliar. </p>
<p>El tema de mi tesis fueron los peces de la Cuenca del Río San Juan afluente del río Bravo. En la Cuenca del Río Bravo se han registrado alrededor de 145 especies de peces de 19 familias distintas.  </p>
<p>Cuando terminé la carrera trabajé como piscicultor en el Banco Nacional de Crédito Ejidal. Me hubiera encantado desarrollar criaderos con especies nativas pero la burocracía no lo permitió. En 1962 salí a estudiar el doctorado a la Universidad de Tulane en Nueva Orleans. </p>
<p> De regreso en México ingresé al Departamento de Vertebrados de la Universidad de Nuevo León donde fundé el Laboratorio de Ictiología. Entre mis estudiantes estuvo la Dra. María de Lourdes Lozano-Vilano. En México hice estudios desde Baja California hasta Quintana Roo. Un lugar muy especial para investigar fue Cuatro Ciénegas en Coahuila donde pude trabajar al lado de los biólogos Eglantina Canales y Julio Carrera. Al darnos cuenta de la importancia de este sitio iniciamos su promoción en 1967 y finalmente se declaró protegido en 1994. </p>
<p>Mi hermano Armando Jesús, mi hijo Salvador Contreras Arquieta y mi sobrino Jorge Contreras Lozano también se dedicaron a la biología.</p>
<p>A pesar de que mi gran amor fueron los peces, también incursioné en el conocimiento de otros grupos de organismos como las orquídeas, las plantas acuáticas, los isópodos cavernícolas y los acociles. Conociendo la problemática del agua en el norte de México me involucré en los estudios de impacto y restauración ambiental. Fui pionero en el estudio de <a href="https://www.biodiversidad.gob.mx/especies/Invasoras"> invasiones acuáticas </a>en México y documenté varios casos en el país. Además, fui miembro fundador y primer presidente de la Sociedad Ictiológica Mexicana, A.C. Fallecí el 11 de febrero de 2009 en San Nicolás de los Garza, Nuevo León.</p>
										  </section>

								</div>
								<div class="3u 12u(mobile)">

									<!-- Right Sidebar -->
                                  <section>
											<header>
												<h2>Mis favoritos</h2>
										  </header>
<ul class="link-list">
			  <li><img src="../images/pagInternas/SalvadorContImg1.jpg" width="230" height="173" alt=""/></li>
			  <li>Carpita negra <em>(Cyprinella ornata)</em></li>
			  <li>Foto: Omar Domínguez Domínguez / Banco de imágenes, CONABIO</li>
			  </ul>
                                    </section>
                                  <section>
											<header>
												<h2>Mis lugares</h2>
										  </header>
<ul class="link-list">
               	<li><img src="../images/pagInternas/SalvadorContImg2.jpg" width="330" height="200" alt=""/></li>
           		<li>Cuatro Ciénegas, Coahuila, Foto: Iván Montes de Oca / CONABIO</li>
              </ul>
				</section>
                                  <section>
									<header>
									  <h2>Mis ideas</h2>
									</header>
<ul class="link-list">
		      <li><img src="../images/pagInternas/SalvadorContImg3.jpg" width="230" height="330" alt=""/></li>
              <li></li>
			  </ul>
				</section>
                                  <section>
										  <header>
										    <ul class="link-list">
										      <li><img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <img src="../assets/css/images/bg02.jpg" width="5" height="5" alt=""/> <a href="http://www.biodiversidad.gob.mx/pais/curiosos">Curiosos y comprometidos</a></li>
										    </ul>
										  </header>
								  </section>
							  </div>
							</div>
						</div>
					</div>
				</div>

			<!-- Footer -->
				<div id="footer-wrapper">
					<footer id="footer" class="container">
						
								<!-- Links -->
                                
							
					</footer>
				</div>
			
		<?php include '../Layouts/footer.php'?>